...
void input_booster_init(void)
{
    ...
    pr_booster("[Input Booster] %s   ndevice_in_dt : %d\n", __FUNCTION__, ndevice_in_dt);
    ...
    if(device_tree_infor > 0) {
        ...
        for_each_child_of_node(np, cnp) {
            ...
            if (plevels && nlevels) {
                ...
                pr_booster("[Input Booster] %s   dt_infor->nlevels : %d\n", __FUNCTION__, dt_infor->nlevels);
            } else {
                pr_booster("Failed to calculate number of frequency.\n");
                break;
            }
            ...
            if (!dt_infor->param_tables) {
                pr_booster("Failed to allocate memory of freq_table\n");
                break;
            }
            ...
            for (i = 0; i < dt_infor->nlevels; i++) {
                ...
                DTSI_TO;
                if (err) {
                    pr_booster("Failed to get [%d] param table property\n", i);
                }
/*
                pr_booster("[Input Booster] Level %d : frequency[%d,%d,%d,%d] hmp_boost[%d] times[%d,%d,%d]\n", i,
